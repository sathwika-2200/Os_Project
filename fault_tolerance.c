...
void ft_heartbeat_check(void) {
  ...
  for (int i = 0; i < g_node_count; i++) {
    ...
    if (n->status == STATUS_ONLINE) {
      snprintf(msg, sizeof(msg), "Heartbeat OK  ← %.7s  [load: %d%%  files: %d]",
               n->id, n->load, n->file_count);
      LOG_OK(msg);
    } else {
      snprintf(msg, sizeof(msg), "No heartbeat  ← %.7s  - NODE DOWN!", n->id);
      LOG_ERR(msg);
      dead++;
    }
  }
  ...
  if (dead == 0) {
    ...
    char msg[80];
    snprintf(msg, sizeof(msg), "%d node(s) unresponsive. Run Rebuild Replicas.",
             dead);
    LOG_WARN(msg);
  }
...
void ft_rebuild_replicas(void) {
  ...
  for (int i = 0; i < g_file_count; i++) {
    ...
    char info[128];
    snprintf(info, sizeof(info), "'%s': active replicas=%d, need %d more.",
    ...
    for (int j = 0; j < g_node_count && needed > 0; j++) {
      ...
      /* add file to candidate node */
        if (cand->file_count < MAX_FILES) {
            store_write(cand->id, fe->filename, fe->content);
            strncpy(cand->files[cand->file_count++], fe->filename,
                    MAX_FILENAME - 1);
            cand->load = (cand->file_count * 100) / MAX_FILES;
        }
      ...
      snprintf(msg, sizeof(msg), "Rebuilt replica of '%.63s' -> %.7s", fe->filename,
               cand->id);
      LOG_OK(msg);
  ...
  if (total_rebuilt == 0) {
    ...
    char msg[64];
    snprintf(msg, sizeof(msg), "Rebuilt %d replica(s) to meet RF=%d.",
...
/* -- Heartbeat Thread Logic ------------------------------------ */

static void *heartbeat_worker(void *arg)
{
    (void)arg;
    while (1) {
        /* Sleep for 10 seconds without warnings */
        sleep_ms(10000);
        
        /* Run a silent heartbeat check in background */
        /* To avoid clobbering the main menu, we'll just log to a background state 
           or use a subtle log message */
        time_t now = time(NULL);
        struct tm *tm_info = localtime(&now);
        char ts[10];
        strftime(ts, sizeof(ts), "%H:%M:%S", tm_info);
        
        /* Check node status and update load if needed */
        for (int i = 0; i < g_node_count; i++) {
            if (g_nodes[i].status == STATUS_ONLINE) {
                /* Simulate minor load fluctuations */
                int flux = (rand() % 5) - 2;
                g_nodes[i].load += flux;
                if (g_nodes[i].load < 5) g_nodes[i].load = 5;
                if (g_nodes[i].load > 95) g_nodes[i].load = 95;
            }
        }
    }
    return NULL;
...
{
    pthread_t thread_id;
    if (pthread_create(&thread_id, NULL, heartbeat_worker, NULL) != 0) {
        LOG_ERR("Failed to create heartbeat thread.");
    } else {
        pthread_detach(thread_id);
        LOG_INFO("Background heartbeat thread started.");
    }
}
